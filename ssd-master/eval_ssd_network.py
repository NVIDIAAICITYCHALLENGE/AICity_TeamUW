...
# =========================================================================== #
tf.app.flags.DEFINE_integer(
    'num_classes', 15, 'Number of classes to use in the dataset.')
tf.app.flags.DEFINE_integer(
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/device:CPU:0'):
            ...
            tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
            aa = list(aps_voc07.values()) 
            for kk in range(len(aps_voc07)):
                op = tf.summary.scalar('1',aa[kk],collections=[])
                op = tf.Print(op, [aa[kk]], summary_name)
                tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
