...
BOOL CIndVehicleTracking::IndVehicleTracking_Process(int iCameraID, BYTE* pbCurrentFrameRGB24, __int64_t iTimeStamp)
{
	...
	// do tracking
	m_GlobalTimeStamp = __int64_t(iTimeStamp);
...
bool CIndVehicleTracking::initSurCamInfo(SurCamInfo& sur_cam, const char* inifile_path)
{
	...
	case PROJECTIVE:
		{
			...
			strcpy(InExCamParamPath, ini.readStr("InExCamParamPath","InExCamParam.txt").c_str());
	...
	case BASIC_BG_SUB:
		{
			...
			if (bg_mode = LOAD_IMG) {
				strcpy(ImgPath, ini.readStr("BgImgPath","background.bmp").c_str());
	...
	sur_cam.setBeginFrame(__int64_t(ini.readInt("BeginFrame", 0)));
